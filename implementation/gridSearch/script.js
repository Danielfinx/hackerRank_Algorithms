...
function gridSearch(G, P) {
  ...
  //I think this solution of four (4!) nested for... loops
  //is pretty inefficient, but hey, it works :)

  gOuter: for (let gi = 0; gi < G.length; gi++) {
    for (let gj = 0; gj < G[0].length; gj++) {
      pOuter: for (let pi = 0; pi < P.length; pi++) {
        for (let pj = 0; pj < P[0].length; pj++) {
          if (G[gi + pi][gj + pj] !== P[pi][pj]) {
            break pOuter;
          } else if (pi === P.length - 1 && pj === P[0].length - 1) {
            patternExist = true;
            break gOuter;
          }
        }
  ...
  return patternExist ? "YES" : "NO";
}

for (let sample of sampleInput) {
  console.log(gridSearch(...sample));
}
